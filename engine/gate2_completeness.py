...
def pipe_thread_designation_ok(size, radii):
    ...
    範囲照合(`pipe_thread_envelope`)だけだと、隣の呼びでも範囲が重なって通ることがある。
    ねじ穴の輪郭群のうち **最大径が山径(=めねじの谷径 d)以下ではない**こと・
    **2番目に大きい径(下穴/谷径)が外径 d を超えない**ことを併せて要求すると、
    呼びが1つに絞られる(盲検 25154-3-04 の Rc1/8。Rc1/16・Rc1/4 は落ちる)。
...
def polygon_covered(poly, widths, diameters, oblique_dims=()):
    ...
    **斜め線形寸法(`measure.direction` に角度を書いた寸法)** を受け付ける。
    ...
    for od in oblique_dims:
        if _oblique_matches_polygon(poly, od):
            return _oblique_matches_polygon(poly, od)
    return None
...
def _oblique_matches_polygon(poly, od):
    ...
    誤認すると、多角形が寸法無しで合格してしまう)。測定点の実体まで一致を要求する。
    """
...
def check_completeness(dxf_path, plan_path, drop_dim_ids=(), verbose=False):
    ...
    # ❗F1: 位置ノードが「どの線分から生まれたか」を控えておく。多角形として1特徴化した
    #   頂点を**軸の対称判定から除外**するのに使う(斜めに付いた六角座1個で軸全体が
    #   非対称と判定され、対寸法で解けるはずのチェーンが解けなくなるのを防ぐ)。
    ...
    #   外さないと、斜めに付いた六角座1個で軸全体が非対称と判定され、対寸法(150/140/100/50)
    #   で解けるはずの位置チェーンが解けなくなる(盲検 25154-1-04 のX軸。修理パス報告 F1)。
