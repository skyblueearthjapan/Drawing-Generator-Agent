...
```jsonc
{
  ...
  "purpose": null,                    // 省略可。"style"=様式寸法(§2.7)
...
- 実測は `dim_engine.measure_angle_deg()` が defpoint から再計算する。
  値は **p1 から p2 へ反時計回り(CCW)に測った角**で、**実際に描かれる寸法円弧そのもの**。
- ❗❗**2026-08-11 修正(重大な作図欠陥)**: ezdxf の `add_angular_dim_2l` は
  `start_angle = line2 の向き / end_angle = line1 の向き` としてCCWに円弧を描くため、
  旧実装(`line1=(v,p1), line2=(v,p2)`)は**常に優角(360−θ)側**に円弧を描いていた。
  値と寸法文字は正しいのに**円弧だけが反対回り**なので、ゲート①も独立検証も素通りし、
  25154-3-09 は「30度」と書かれた約310度の円弧を持ったまま納品されていた。
  現在は (a) 呼び出しの line1/line2 を入れ替え (b) `measure_angle_deg` が
  **描かれた円弧**を返し (c) `dim_engine.check_angle_arc()` が
  **アノニマスブロック内のARC実体**を読んで「円弧の中点が測定セクタ内にあるか」
  「円弧長が測定角±矢印逃げに収まるか」を検査する。
  **計画JSONの書き方(p1→p2 のCCW)と値は従来と同じ**で、直す必要はない
  (25154-3-09 は再生成して 30度の円弧に修正済み)。反証は
  `調査/run_cycle3_falsification.py` の H7/H8。
...
"placement": {
  ...
  "chain_group": null    // 省略可。同じ名前の寸法を1本の寸法線へ直列連記する(§2.3a)
}
```
...
- **4方向すべてが実際に効く**(反証テスト `調査/run_style_falsification.py` の S9 で、
  4方向の寸法線位置が `輪郭 ± オフセット` と一致することを実DXFから検算している)。
  径寸法の左右振り分け(作法6)・径の帯と長さの帯の分離(作法7)はこの `side` で書く。
...
#### 2.3a `chain_group` — 直列連記(1本の寸法線に複数寸法を連ねる)【2026-08-11 追加】

人間コーパスは「端点が接する寸法を同一寸法線へ連記する」直列(chain)を
**水平23%・垂直17%**の頻度で使うのに対し、生成図面は**0%**だった
(`調査/drawing_style_analysis.md`§6。原因は「2段目以降+8mm刻み」で必ず別段へ積む実装)。
`placement.chain_group` に同じ名前を書いた寸法は、**1本の寸法線上に並ぶ**。

```jsonc
{"id": "Z70_5_body", "placement": {"side": "below", "chain_group": "Zseg"}, ...}
{"id": "Z3_spigot",  "placement": {"side": "below", "chain_group": "Zseg"}, ...}
// -> 下側の1本の寸法線に「70.5 | 3」が直列で並ぶ(旧: 16mm と 24mm の2本に分かれていた)
```

- グループは**ビュー内で閉じる**(同じ名前でもビューが違えば別グループ)。
- オフセットは **グループ内の最大値に揃えられる**(`level` を書き分けても連記は壊れない)。
  レイアウト側の予約帯(§1.3a)も同じ値を見るので、ビュー間隔は自動で追随する
  (計算は `compose_drawing.resolve_dim_offsets` の1実装に集約してある)。
- **エンジンが実DXFの defpoint から整列を検証し、破れていたらゲート①で不合格にする**
  (`dim_engine.check_chain_alignment`。独立検証 `generate_drawing.independent_verify` でも
  保存済みDXFから測り直す)。検査項目:

  | # | 落ちる条件 |
  |---|---|
  | 1 | メンバーが2本未満 |
  | 2 | 測定方向(寸法のangle)が揃っていない |
  | 3 | `side` が混在している |
  | 4 | 寸法線が同一直線上にない(=連記が崩れた) |
  | 5 | 測定区間が互いに重なる |
  | 6 | 隣り合う区間が端点を共有しない(飛び地の並記) |

  反証は `調査/run_style_falsification.py`(S0〜S9。**オフセット決定を旧実装へ差し替える
  実装退行の注入**を含めて9/9検出)。

> ❗**直列連記は「同じ辺・同じ方向・端点を共有する連続区間」専用**。全長のような
> 区間が重なる寸法は連記できないので、`level` を1段外側にして別の寸法線に置く。
...
### 2.7 `purpose: "style"` — 様式寸法【2026-08-11 追加】

**形状の一意性のためではなく、読み手への配慮のために入れる寸法**を明示する印。
現在の用途は**等配穴群の位相(基準角)を示す角度寸法**(作法9)と、面取り角の明示。

```jsonc
{"id": "A20_phase", "kind": "angle", "purpose": "style", ...}
```

- ゲート②は位相を**判定対象外**にしている(`find_pcd_groups` は個数・PCD径・円周等分だけを
  実ジオメトリで検算し、位相は `out_of_scope`)。したがって位相の角度寸法は
  「どの特徴にも結び付かない」ため **必ず『宙に浮いた寸法』警告になってしまい、
  人間が実際にやっている作図を計画側が避ける動機になっていた**。
- `purpose:"style"` を付けると、その寸法は
  **`floating_dimensions`(警告)から外れ、`out_of_scope` に `class:"style_dimension"` として
  明示的に列挙される**。
- ❗**完全性の判定は一切甘くならない**: 様式寸法はカバレッジの根拠に使われないので、
  未指定件数・合否は `purpose` の有無で1件も変わらない
  (反証 `調査/run_style_falsification.py` の S8 が、同じDXFに対し `purpose` 有無で
  `unspecified` が完全一致し `floating` だけが消えることを機械確認する)。
- ゲート①(値・文字・円弧)は様式寸法にも**通常どおり全部かかる**。
