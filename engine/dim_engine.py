...
def is_oblique_direction(angle):
    u"""測定方向が図面の軸(0/90度)に平行でないか。"""
    a = float(angle) % 180.0
    return min(abs(a - 0.0), abs(a - 90.0), abs(a - 180.0)) > 1e-6


def oblique_base_point(side, p1, p2, angle, offset):
    u"""斜め線形寸法(回転した正多角形の対角など)の寸法線位置。

    軸平行寸法はビュー外周から offset だけ外へ出すが、斜め寸法は外周に対する
    「上下左右」が定義できない。**測定点の中点から測定方向に垂直な向きへ offset**
    だけ離す(向きの符号は placement.side で決める)。
    フィーチャーの真横に置かれるので、ビュー外周の寸法予約帯には算入しない
    (`compose_drawing.plan_view_reserves` 側も同じ判定で除外している)。
    """
    ...
    for item in plan.get("dimensions", []):
        ...
        if kind in ("linear", "diameter_linear"):
            ...
            if is_oblique_direction(angle):
                # 斜め線形寸法(回転した正多角形の対角=二面幅系。ゲート②E4)。
