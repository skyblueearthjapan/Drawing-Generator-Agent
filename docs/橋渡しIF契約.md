...
```jsonc
{
  ...
  "品名": "string",        // 省略可(省略時は生成物ファイル名にモデルファイル名由来の値を使う)
  "装置名": "string",      // 省略可
...
- 橋渡し側のパース規約(推奨): 正規表現 `^topic:\s*(.+)$` を**ファイル先頭数行**の範囲で探す
  (厳密に2行目固定ではなく、AIオペレータの出力ゆれを見込んで先頭5行程度を許容範囲にすること)。
  `topic:` 行が無い質問票(旧形式・移行期の書き漏れ)も有効な質問票として扱い、
  topicは「不明」等のフォールバック値にする(必須にしすぎて質問票自体を捨てない)。
